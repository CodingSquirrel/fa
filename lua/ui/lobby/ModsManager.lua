...
-- The exclusive mod selected, if any
local exclusiveMod = nil
...
function Refresh_Mod_List()
        ...
        elseif mod.enabled == false then
            ...
            elseif dependencies.requires then
                ...
                for k, v in dependencies.requires do
                    if not modBackwardDependencyMap[v] then
                        modBackwardDependencyMap[v] = {}
                    end

                    modBackwardDependencyMap[v][uid] = true
                end
            end
    ...
    -- each.
    local function appendMods(modlist, active, enabled, label, labelSet)
        for k, v in modlist do
            label = label or LOC(labelSet[k])
...
function DeactivateMod(uid)
    ...
    if victims then
        -- Dependency cycles are not permitted: we break those at initialisation time.
